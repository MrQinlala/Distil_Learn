...
def distillation_loss_function(teacher_logits, student_logits, 
    ...
    #    让学生在真实标签上也保持一定的准确度
    #    -100 表示填充位置不计算loss
    assert not torch.isnan(student_logits).any(), "student_logits包含NaN"  
...
parser.add_argument("--plot_name",default = 'dl_origin_first.png',help = "损失曲线文件名")
...
for epoch in range(num_epochs):
    ...
    for step, batch in enumerate(train_loader):
        ...
        if step % 100 == 0:
            print(f"Epoch {epoch}, Step {step}, Loss {loss.item():.4f}, LM {lm_loss.item():.4f}, KD {distill_loss.item():.4f},Totol_loss{total_loss_val:.4f}")
